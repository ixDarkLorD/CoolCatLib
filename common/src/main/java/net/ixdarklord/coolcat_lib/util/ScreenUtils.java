...
public class ScreenUtils {
    ...
    public static void enableScissor(int minX, int minY, int maxX, int maxY) {
        ...
        double e = (double)minX * d;
        double f = (double)m - (double)maxY * d;
        double g = (double)(maxX - minX) * d;
        double h = (double)(maxY - minY) * d;
    ...
    public static Component limitComponent(Component component, int maxWidth) {
        ...
        return component.getString().length() >= maxWidth
                ? new TextComponent(text.substring(0, text.length() - 3) + "...").withStyle(component.getStyle())
                : component;
    }
    ...
    public static List<Component> splitComponent(Component component, int maxWidth) {
        ...
        return font.getSplitter().splitLines(component, maxWidth, component.getStyle()).stream()
                .map(text -> new TextComponent(text.getString()).withStyle(component.getStyle()))
