...
public abstract class AbstractScrollableWidget extends AbstractDraggableWidget {
    ...
    protected void renderBackground(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
        if (this.bgSprites != null)
            guiGraphics.blit(this.bgSprites.get(this.visible, this.isFocused()), this.x, this.y, 0, 0, this.width, this.height);
        else {
    ...
    protected void renderScrollbar(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        if (barSprites != null) {
            ...
            guiGraphics.blit(barSprites.get(this.canScrollVertically(), false), this.getScrollbarPosition().x(), relativeY, 0, 0, this.scrollWidth, this.scrollHeight);
            return;
        }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        ...
        if (delta != 0) {
            this.scrollOffsetY += (float) (-delta * this.getScrollStep(ScreenAxis.VERTICAL));
            this.applyScrollLimits();
            return true;
        }
        return false;
